              ...
              <div key={laneIndex} className="flex items-center gap-2">
                ...
                <div className="flex-1 grid grid-cols-5 gap-2">
                  {Array(5).fill(0).map((_, tileIndex) => {
                    const isRevealed = laneIndex < currentLane || (laneIndex === currentLane && revealedTiles[laneIndex] === tileIndex) || (gameOver && laneIndex <= currentLane + 1);
                    ...
                    return (
                      <button
                        ...
                        className={`
                          ...
                          ${canClick ? 'hover:scale-105 hover:bg-emerald-600 cursor-pointer bg-emerald-700 border-2 border-emerald-500' : ''}
                          ${isRevealed && isTrap ? 'bg-red-600' : ''}
                          ${isRevealed && !isTrap && isCurrentPath ? 'bg-emerald-500' : ''}
                          ${isRevealed && !isTrap && !isCurrentPath && laneIndex <= currentLane ? 'bg-gray-600' : ''}
        ...
        <div className="bg-black/30 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                {(Object.keys(difficultyConfig) as Difficulty[]).map((diff) => (
                  <Button
                    ...
                    disabled={isPlaying}
                    className={difficulty === diff ? "bg-emerald-600 hover:bg-emerald-700" : "border-gray-600 text-white hover:bg-gray-700"}
                  >
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
            <div className="text-white">
              ...
              <span className="text-xl font-bold text-emerald-400">${balance.toFixed(2)}</span>
            </div>
            
            {!isPlaying ? (
              <Button onClick={startGame} size="lg" className="bg-emerald-600 hover:bg-emerald-700 w-full sm:w-auto px-12">
                Play
              </Button>
            ) : (
              <div className="flex gap-4 items-center w-full sm:w-auto">
                <div className="text-center">
                  ...
                  <p className="text-xl font-bold text-emerald-400">{getCurrentMultiplier().toFixed(2)}x</p>
                </div>
                <Button 
                  onClick={cashOut} 
                  size="lg" 
                  disabled={currentLane < 0}
                  className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold flex-1 sm:flex-none px-8"
                >
